...
class CalibNN(nn.Module):
    ...
    def forward(self, x, meta):
        x_embeds, encoder_hidden = self.emb_model(x, meta)
        ...
        # print(x.shape, Hi_data.shape, x_embeds.shape) # torch.Size([3, 175, 5]) torch.Size([3, 33, 1]) torch.Size([3, 64])
        # input()
        # print(encoder_hidden.shape) # torch.Size([4, 3, 32])
        ...
        # print(out.shape, x.shape) # torch.Size([3, 33, 3]) torch.Size([3, 175, 5]) # 3 parameters for each week, there are 33 weeks (29 + 4 ahead)
        # input()
        return out
...
def param_model_forward(param_model,params,x,meta):
    ...
    if params['model_name'].startswith('GradABM-time-varying'):
        action_value = param_model(x, meta)  # time-varying
        # print(x.shape, action_value.shape) # torch.Size([3, 175, 5]) torch.Size([3, 33, 3])
        # input()
...
def build_simulator(params,devices,counties):
    ...
    if 'ABM' in params['model_name']:
        if params['joint']:
            ...
            # build GRADABM models for each country ('25001', '25005', '25023')
...
def forward_simulator(params,param_values,abm,training_num_steps,counties,devices):
    ...
    if params['joint']:
        ...
        # print(training_num_steps) # 175
        # input()
        # param_values = param_values.permute(1, 0, 2)
        # print(param_values.shape)
        for time_step in range(training_num_steps):
            if 'time-varying' in params['model_name']:
                param_t = param_values[:,time_step//7,:] # get the corresponding parameter values since here time_step is by day, but param_values are by week.
            else:
...
def runner(params, devices, verbose):
    for run_id in range(params['num_runs']):
        ...
        # print(training_num_steps, params['num_steps']) # 175 203
        # input()
        ...
        if train_flag:
            ...
            # if not ModuleValidator.is_valid(param_model):
            #     print("---fixing---")
            #     param_model = ModuleValidator.fix(param_model)


            # param_model, opt, train_loader = privacy_engine.make_private(
            #     module=param_model,
            #     optimizer=opt,
            ...
            param_model, opt, train_loader = privacy_engine.make_private_with_epsilon(
                module=param_model,
                optimizer=opt,
                data_loader=train_loader,
                target_epsilon=target_epsilon, # total epsilon
                target_delta=1E-3, # 175 samples,  1/175
                max_grad_norm=1.0, # sensitivity
                epochs=num_epochs
            )
            ...
            for epi in range(num_epochs):
                ...
                for batch, data in enumerate(train_loader): 
                    ...
                    # print(x.shape) # 3, 175, 5
                    # print(X_train_dim) 5
                    # print(seqlen) 175
                    # print(y.shape) # 3, 175, 1
                    # print(x[0, :10, :], y[0, :10, :])
                    # input()
                    # print(param_values.shape) # [3,33,3] 33 is equal to 29 + 4, where 29 is the number of weeks of days of 203, and 4 is ahead time. 这里设置成29就行了吧，感觉不需要设置成33
                    # input()
                    if verbose:
                ...
                epsilon = privacy_engine.accountant.get_epsilon(delta=1E-3)

                printstr = f" | (ε = {epsilon:.2f}, δ = {1E-3})"
