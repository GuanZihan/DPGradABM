...
class EmbedAttenSeq(nn.Module):
    ...
    def forward(self, seqs, metadata):
        # print(seqs.shape) # torch.Size([3, 175, 5])
        # Take last output from GRU
        latent_seqs, encoder_hidden = self.rnn(seqs)
        # encoder_hidden = encoder_hidden.transpose(0,1)
        # print(seqs.shape, encoder_hidden.shape, latent_seqs.shape) # torch.Size([175, 3, 5]) torch.Size([4, 3, 32]) torch.Size([175, 3, 64]) -> torch.Size([3, 175, 5]) torch.Size([3, 4, 32]) torch.Size([3, 175, 64])
        # input()
        # print(latent_seqs.shape, metadata.shape)
        # print(self.attn_layer(latent_seqs).shape)
        latent_seqs = self.attn_layer(latent_seqs).sum(1)
        # print(latent_seqs.shape, metadata.shape) # torch.Size([3, 64]) torch.Size([3, 10])
        # input()
        out = self.out_layer(torch.cat([latent_seqs, metadata], dim=1))
...
class DecodeSeq(nn.Module):
    ...
    def forward(self, Hi_data, encoder_hidden, context):
        ...
        # print(inputs.shape, context.shape) # 3,33,64
        context = torch.unsqueeze(context, 1)
        # repeat context for each item in sequence  
        context = context.repeat(1,inputs.shape[1],1)
        ...
        # print(inputs.shape, h0.shape)
        # input("======")
        
        latent_seqs = self.rnn(inputs, h0)[0]
        # latent_seqs = latent_seqs.transpose(1,0)
        # print(latent_seqs.shape, "=====") #torch.Size([33, 3, 64]) =====
...
def fetch_county_data_covid(state='MA', county_id='25005', pred_week='202021', batch_size=32, noise_level=0):
    ...
    for county in all_counties:
        ...
        y = moving_average(y[:,1].ravel(),SMOOTH_WINDOW).reshape(-1,1) # why?
        c_seqs.append(X_county.to_numpy())
        c_ys.append(y)
        # print(X_county.shape, y.shape)
        # input()
        # print(y)
        # print(X_county)
        # input()
